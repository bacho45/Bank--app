    ...
    function incOrExp(textColor, borderColor, text) {
      ...
      user.innerHTML = `
        <div class='d-flex justify-content-between m-2' id= '${element.id}'>
          <p class="m-0">Date: ${element.date}</p>
          <button type='button' class='border-0 bg-white btn-close delButton'></button>
        </div>
        <p class="m-2">Category: ${element.category}</p>
        <p class="m-2">Amount: ${element.amount}$</p>
        <div class='d-flex justify-content-between m-2'>
          <p class="m-0 ${textColor}">${text}</p>
          <button type='button' class='border-0 bg-white text-primary editButton'>Edit</div>
        </div>`;
    }

    if (element.type === "income") {
      incOrExp("text-success-emphasis", "border-success-subtle", "Income");
    }

    if (element.type === "expense") {
      incOrExp("text-danger-emphasis", "border-danger-subtle", "Departure");
    }
    datas.append(user);
...
function renderTotal(arr) {
  ...
  total.innerHTML = `
  <p class="m-2">Income:<span class='ps-2 text-success-emphasis'>
  ${sumInc}$</span></p>
  <p class="m-2">Departure:<span class='ps-4 text-danger-emphasis'>
  ${sumExp}$</span></p>
  `;
}
